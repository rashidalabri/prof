...
_W = flags.DEFINE_string(
    "w", "10", "The W value for optimization."
)

_B = flags.DEFINE_string(
    "b", "10", "The B value for optimization."
)

_NUM_REPS = flags.DEFINE_string(
    "num_reps", "5", "Number of times to repeat the optimization process."
)
...
def main(_):
  ...
  w_true = int(_W.value)  # the true w
  b_true = int(_B.value)  # the true b
  max_num_steps = 500  # the number of optimization steps
  num_reps = int(_NUM_REPS.value)  # the number of repeated runs
  ...
  if optimizer_llm_name in {"gpt-3.5-turbo", "gpt-4", "gpt-4o", "gpt-4o-mini"}:
  ...
  if optimizer_llm_name.lower() == "text-bison":
    ...
    assert optimizer_llm_name in {"gpt-3.5-turbo", "gpt-4", "gpt-4o", "gpt-4o-mini"}
  ...
  Now you will help me minimize a function with two input variables w, b. I have some (w, b) pairs and the function values at those points. The pairs are arranged in descending order based on their function values, where lower values are better.
    """.strip()
    feedback_prompt += "\n\n"
    feedback_prompt += old_value_pairs_substr.strip()
    feedback_prompt += "\n\n"
    ...
    # )
    # meta_prompt += "\n\n"
    feedback_prompt += f"""Given this proposed input value(s): {point} and its corresponding function value {score}, give one to three sentences of feedback of the proposed input value.
  As part of your feedback, please compare the proposed solution’s score to the best solutions in the previously provided list and also compare it to the closest matching input (make sure they are not exactly the same, however).
  Additionally, explain how the score has changed relative to that closely related solution, and based on these observations, clarify whether it makes sense to keep refining possible solutions similar in nature to the proposed solution or to consider different approaches.
  Furthermore, give information as to how the input should change (considering the direction and magnitude of the differences in scores that you observe).
  Be sure to adhere to these directions, and again keep your response between one and three straightforward sentences.
    """.strip()
  ...
  Now you will help me minimize a function with two input variables w, b. I have some (w, b) pairs and the function values at those points. The pairs are arranged in descending order based on their function values, where lower values are better.
    ...
    # meta_prompt += "\n\n"
    if feedback != "":
      meta_prompt += """Additionally, consider this feedback from a previous iteration of this process:\n"""
  ...
  for i_rep in range(num_reps):
    ...
    for i_step in range(max_num_steps):
      ...
      # for point, score in zip(rounded_outputs, single_step_values):
      ...
      #       score=score
      #   )
      #   feedback += f"(w={point[0]}, w={point[1]}): value={score}\n"
      #   raw_feedback = call_optimizer_server_func(feedback_prompt)
      #   feedback += raw_feedback[0]
      #   feedback += '\n\n\n'
      # ====================== save results ============================
